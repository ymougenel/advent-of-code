...
def add_rock(id):
    ...
    required_space = highest_rock + room_required + 4 - len(chamber)
    if required_space > 0:
    ...
    for i in range(4):
        for k in range(7):
            if dist >= 1:
                ...
                current = k
            if position - 3 + i >= 0 and 0 <= current + dist < 7 and chamber[position - 3 + i][current] == id:
                chamber[position - 3 + i][current] = "."
                chamber[position - 3 + i][current + dist] = id
